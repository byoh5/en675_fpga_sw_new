...
void KeyScan(void)
{
	...
	gbSKeyVal =	(nADC_KEY<(((UINT)UPr(UpADC_LEVEL_C)+(UINT)UPr(UpADC_LEVEL_U))>>1)) ? KEY_VAL_C :		// Center
				(nADC_KEY<(((UINT)UPr(UpADC_LEVEL_U)+(UINT)UPr(UpADC_LEVEL_D))>>1)) ? KEY_VAL_U :		// Up
				(nADC_KEY<(((UINT)UPr(UpADC_LEVEL_D)+(UINT)UPr(UpADC_LEVEL_L))>>1)) ? KEY_VAL_D :		// Down
				(nADC_KEY<(((UINT)UPr(UpADC_LEVEL_L)+(UINT)UPr(UpADC_LEVEL_R))>>1)) ? KEY_VAL_L : 	// Left
//					(nADC_KEY<(((UINT)UPr(UpADC_LEVEL_R)+             0x000001ff)>>1)) ? KEY_VAL_R : 0;	// Right
				(nADC_KEY<(((UINT)UPr(UpADC_LEVEL_R)+             0x000003ff)>>1)) ? KEY_VAL_R : 0;	// Right
	...
	KeyCht(gbSKeyVal, &gtSKey, (1<<UPr(UpAD_CHT_BIT))-1);
