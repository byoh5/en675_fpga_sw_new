...
RetCode Wave5VpuInit(Uint32 coreIdx, void* firmware, Uint32 size)
{
    ...
    //VLOG(INFO, "\nVPU INIT codeBase %x firmware %x!!!\n", codeBase, firmware);
...
RetCode Wave5VpuDecRegisterFramebuffer(CodecInst* inst, FrameBuffer* fbArr, TiledMapType mapType, Uint32 count)
{
    ...
    if (mapType >= COMPRESSED_FRAME_MAP) {
        ...
        if (pDecInfo->productCode == WAVE521C_DUAL_CODE) {
            ...
            case W_AV1_DEC:     fbcYTblSize = WAVE5_FBC_LUMA_TABLE_SIZE(initPicWidth, initPicHeight); break;
            default:
                return RETCODE_NOT_SUPPORTED_FEATURE;
            }
        ...
        if (pDecInfo->productCode == WAVE521C_DUAL_CODE) {
            ...
            case W_AV1_DEC:     fbcCTblSize = WAVE5_FBC_CHROMA_TABLE_SIZE(initPicWidth, initPicHeight); break;
            default:
                return RETCODE_NOT_SUPPORTED_FEATURE;
            }
...
RetCode Wave5VpuDecode(CodecInst* instance, DecParam* option)
{
    ...
    VpuWriteReg(instance->coreIdx, W5_USE_SEC_AXI,  0);
#endif
...
Int32 Wave5VpuWaitInterrupt(CodecInst* instance, Int32 timeout, BOOL pending)
{
    ...
    VLOG(INFO, "Wave5VpuWaitInterrupt : int_reason = 0x%x\n", reason);
...
RetCode Wave5VpuClearInterrupt(Uint32 coreIdx, Uint32 flags)
{
    ...
    VLOG(INFO, "Wave5VpuClearInterrupt : flags = 0x%x\n", flags);
...
RetCode Wave5VpuEncInitSeq(CodecInst* instance)
{
    ...
    */
    ...
    if ((/*(rotMirMode != 0) &&*/ instance->codecMode == W_AVC_ENC && !((pEncInfo->rotationAngle == 180) && (pEncInfo->mirrorDirection == MIRDIR_HOR_VER))) && ((pOpenParam->picWidth != alignedWidth) || (pOpenParam->picHeight != alignedHeight)))  // if rot/mir enable && pic size is not 32(16)-aligned, set crop info.
    ...
    VpuWriteReg(coreIdx, W5_CMD_ENC_SEQ_RC_PARAM2, pParam->rcWeightBuf<<8 | pParam->rcWeightParam);
...
#ifdef HLS_TEST
    if (pParam->vuiParam.vuiParamFlags || pEncInfo->openParam.encodeVuiRbsp || pEncInfo->openParam.encodeHrdRbspInVPS || pEncInfo->openParam.encodeHrdRbspInVUI) {
        //*** VUI encoding by host registers ***/
        if (pParam->vuiParam.vuiParamFlags) {
            VpuWriteReg(coreIdx, W5_CMD_ENC_SEQ_SET_PARAM_OPTION, OPT_VUI);
            VpuWriteReg(coreIdx, W5_CMD_ENC_SEQ_VUI_PARAM_FLAGS, pParam->vuiParam.vuiParamFlags);
            VpuWriteReg(coreIdx, W5_CMD_ENC_SEQ_VUI_ASPECT_RATIO_IDC, pParam->vuiParam.vuiAspectRatioIdc);
            VpuWriteReg(coreIdx, W5_CMD_ENC_SEQ_VUI_SAR_SIZE, pParam->vuiParam.vuiSarSize);
            VpuWriteReg(coreIdx, W5_CMD_ENC_SEQ_VUI_OVERSCAN_APPROPRIATE, pParam->vuiParam.vuiOverScanAppropriate);
            VpuWriteReg(coreIdx, W5_CMD_ENC_SEQ_VUI_VIDEO_SIGNAL, pParam->vuiParam.videoSignal);
            VpuWriteReg(coreIdx, W5_CMD_ENC_SEQ_VUI_CHROMA_SAMPLE_LOC, pParam->vuiParam.vuiChromaSampleLoc);
            VpuWriteReg(coreIdx, W5_CMD_ENC_SEQ_VUI_DISP_WIN_LEFT_RIGHT, pParam->vuiParam.vuiDispWinLeftRight);
            VpuWriteReg(coreIdx, W5_CMD_ENC_SEQ_VUI_DISP_WIN_TOP_BOT, pParam->vuiParam.vuiDispWinTopBottom);
        }
        else {
            VpuWriteReg(coreIdx, W5_CMD_ENC_SEQ_VUI_PARAM_FLAGS, 0x0);
        }
        ...
        if (pEncInfo->openParam.encodeVuiRbsp || pEncInfo->openParam.encodeHrdRbspInVPS || pEncInfo->openParam.encodeHrdRbspInVUI) {
            ...
            VpuWriteReg(coreIdx, W5_CMD_ENC_SEQ_VUI_HRD_RBSP_PARAM_FLAG, (pEncInfo->openParam.encodeHrdRbspInVPS<<2)    |
            ...
            VpuWriteReg(coreIdx, W5_CMD_ENC_SEQ_VUI_RBSP_SIZE, pEncInfo->openParam.vuiRbspDataSize);
            VpuWriteReg(coreIdx, W5_CMD_ENC_SEQ_HRD_RBSP_ADDR, pEncInfo->openParam.hrdRbspDataAddr);
            VpuWriteReg(coreIdx, W5_CMD_ENC_SEQ_HRD_RBSP_SIZE, pEncInfo->openParam.hrdRbspDataSize);
        }
        else {
            VpuWriteReg(coreIdx, W5_CMD_ENC_SEQ_VUI_HRD_RBSP_PARAM_FLAG, 0x0);
        }
...
RetCode Wave5VpuEncGetSeqInfo(CodecInst* instance, EncInitialInfo* info)
{
    ...
    dbgprintf("info->minSrcFrameCount %d \n",info->minSrcFrameCount);
    info->maxLatencyPictures    = VpuReadReg(instance->coreIdx, W5_RET_ENC_PIC_MAX_LATENCY_PICTURES);
    dbgprintf("info->maxLatencyPictures %d \n",info->maxLatencyPictures);
    info->vlcBufSize        = VpuReadReg(instance->coreIdx, W5_RET_VLC_BUF_SIZE);
    dbgprintf("info->vlcBufSize %d \n",info->vlcBufSize);
    info->paramBufSize      = VpuReadReg(instance->coreIdx, W5_RET_PARAM_BUF_SIZE);
    dbgprintf("info->paramBufSize %d \n",info->paramBufSize);
...
RetCode Wave5VpuEncode(CodecInst* instance, EncParam* option)
{
    ...
    VpuWriteReg(coreIdx, W5_CMD_ENC_PIC_SRC_AXI_SEL, DEFAULT_SRC_AXI);
    /* Secondary AXI */
    VpuWriteReg(coreIdx, W5_CMD_ENC_PIC_USE_SEC_AXI,  0x0/*(pEncInfo->secAxiInfo.u.wave.useEncRdoEnable<<11) | (pEncInfo->secAxiInfo.u.wave.useEncLfEnable<<15)*/);
...
RetCode Wave5VpuEncParaChange(EncHandle instance, EncChangeParam* param)
{
    ...
    if (param->enable_option & ENC_SET_CHANGE_PARAM_RC2) {
        VpuWriteReg(coreIdx, W5_CMD_ENC_SEQ_RC_PARAM2,  param->rcWeightBuf<<8 | param->rcWeightParam);
...
RetCode CheckEncCommonParamValid(EncOpenParam* pop)
{
    ...
    // Slice
    {
        if ( param->wppEnable == 1 && param->independSliceMode == 1) {
            ...
            if (param->independSliceModeArg % num_ctb_in_width) {
            	dbgprintf("CFG FAIL : If WaveFrontSynchro(WPP) '1', the number of IndeSliceArg(ctb_num) must be multiple of num_ctb_in_width\n");
            	dbgprintf("RECOMMEND CONFIG PARAMETER : IndeSliceArg = num_ctb_in_width * a\n");
                ret = RETCODE_FAILURE;
            }
        }
    ...
    // RDO
#ifdef EYENIX_TEST
    if (param->useRecommendEncParam != 1 && param->cuSizeMode == 0) {
#else
