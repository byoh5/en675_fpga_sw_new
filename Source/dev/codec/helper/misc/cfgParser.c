    ...
    )
{
    ...
    if(strcmp(FileName,"yuv/sub")==0)
    {
    	EYENIX_AVC_TEST = 1 ;
    }
   	strcpy(pEncCfg->SrcFileName, FileName);

    //if (WAVE_GetValue(fp, "SourceWidth", &iValue) == 0)
    //    goto __end_parse;
    //else
        pEncCfg->waveCfg.picX = EYENIX_TEST_WIDTH;//3840;//2048;//1920;//3840;//1920;//416;//iValue;
    //if (WAVE_GetValue(fp, "SourceHeight", &iValue) == 0)
    //    goto __end_parse;
    //else
        pEncCfg->waveCfg.picY = EYENIX_TEST_HEIGHT;//2160;//1600;//1200;//1080;//2160;//1080;//240;//256;////iValue;
    //if (WAVE_GetValue(fp, "FramesToBeEncoded", &iValue) == 0)
    //    goto __end_parse;
    //else
        pEncCfg->NumFrame = 3;//-1;//0;//2;//3;//5;//iValue;
    //if (WAVE_GetValue(fp, "InputBitDepth", &iValue) == 0)
    //    goto __end_parse;
    //else
        pEncCfg->SrcBitDepth = 8;//iValue; // BitDepth == 8 ? HEVC_PROFILE_MAIN : HEVC_PROFILE_MAIN10

    //    goto __end_parse;
    //else
        pEncCfg->waveCfg.internalBitDepth   = 8;//iValue;

    //if (WAVE_GetValue(fp, "LosslessCoding", &iValue) == 0)
    //    goto __end_parse;
    //else
        pEncCfg->waveCfg.losslessEnable = 0;//1;//iValue;
    //if (WAVE_GetValue(fp, "ConstrainedIntraPred", &iValue) == 0)
    //    goto __end_parse;
    //else
        pEncCfg->waveCfg.constIntraPredFlag = 0;//iValue;
    //if (WAVE_GetValue(fp, "DecodingRefreshType", &iValue) == 0)
    //    goto __end_parse;
    //else
        pEncCfg->waveCfg.decodingRefreshType = 0;//iValue;

    //if (WAVE_GetValue(fp, "StillPictureProfile", &iValue) == 0)
    //    goto __end_parse;
    //else
        pEncCfg->waveCfg.enStillPicture = 0;//iValue;

    // BitAllocMode
    //if (WAVE_GetValue(fp, "BitAllocMode", &iValue) == 0)
    //    goto __end_parse;
    //else
        pEncCfg->waveCfg.bitAllocMode = 0;//iValue;
...
/*#define FIXED_BIT_RATIO 49
    ...
    //    goto __end_parse;
    //else
        pEncCfg->waveCfg.scalingListEnable = 0;//iValue;
    ...
    if(EYENIX_AVC_TEST)
    {
		pEncCfg->waveCfg.cuSizeMode = 0;//(intra8&0x01) | (intra16&0x01)<<1 | (intra32&0x01)<<2;
		pEncCfg->waveCfg.cuSizeMode = 0;//1;
    }
    else
    {
		pEncCfg->waveCfg.cuSizeMode = (intra8&0x01) | (intra16&0x01)<<1 | (intra32&0x01)<<2;
		pEncCfg->waveCfg.cuSizeMode = 1;
    }
    //if (WAVE_GetValue(fp, "EnTemporalMVP", &iValue) == 0)
    //    goto __end_parse;
    //else
        pEncCfg->waveCfg.tmvpEnable = 1;//iValue;

    //if (WAVE_GetValue(fp, "WaveFrontSynchro", &iValue) == 0)
    //    goto __end_parse;
    //else
        pEncCfg->waveCfg.wppenable = 0;//iValue;

    //if (WAVE_GetValue(fp, "MaxNumMerge", &iValue) == 0)
    //    goto __end_parse;
    //else
        if(EYENIX_AVC_TEST)
        pEncCfg->waveCfg.maxNumMerge = 0;//iValue;
        else
        pEncCfg->waveCfg.maxNumMerge = 1;//iValue;

    //if (WAVE_GetValue(fp, "EnDBK", &iValue) == 0)
    //    goto __end_parse;
    //else
        pEncCfg->waveCfg.disableDeblk = 0;//!1;//!(iValue);

    //if (WAVE_GetValue(fp, "LFCrossSliceBoundaryFlag", &iValue) == 0)
    //    goto __end_parse;
    //else
        pEncCfg->waveCfg.lfCrossSliceBoundaryEnable = 1;//iValue;

    //if (WAVE_GetValue(fp, "BetaOffsetDiv2", &iValue) == 0)
    //    goto __end_parse;
    //else
        pEncCfg->waveCfg.betaOffsetDiv2 = 0;//iValue;

    //if (WAVE_GetValue(fp, "TcOffsetDiv2", &iValue) == 0)
    //    goto __end_parse;
    //else
        pEncCfg->waveCfg.tcOffsetDiv2 = 0;//iValue;

    //if (WAVE_GetValue(fp, "IntraTransSkip", &iValue) == 0)
    //    goto __end_parse;
    //else
        if(EYENIX_AVC_TEST)
        pEncCfg->waveCfg.skipIntraTrans = 0;//iValue;
        else
        pEncCfg->waveCfg.skipIntraTrans = 1;//iValue;

    //if (WAVE_GetValue(fp, "EnSAO", &iValue) == 0)
    //    goto __end_parse;
    //else
        if(EYENIX_AVC_TEST)
        pEncCfg->waveCfg.saoEnable = 0;//iValue;
        else
        pEncCfg->waveCfg.saoEnable = 1;//iValue;

    //if (WAVE_GetValue(fp, "IntraNxN", &iValue) == 0)
    //    goto __end_parse;
    //else
        if(EYENIX_AVC_TEST)
        pEncCfg->waveCfg.intraNxNEnable = 0;//iValue;
        else
        pEncCfg->waveCfg.intraNxNEnable = 1;//iValue;


    //if (WAVE_GetValue(fp, "RateControl", &iValue) == 0)
    //    goto __end_parse;
    //else
        pEncCfg->RcEnable = 1;//iValue;

    //if (WAVE_GetValue(fp, "EncBitrate", &iValue) == 0)
    //    goto __end_parse;
    //else
        pEncCfg->RcBitRate = 1;//iValue;
    ...
    //else
        if(EYENIX_AVC_TEST)
        pEncCfg->waveCfg.cuLevelRCEnable = 0;//iValue;
        else
        pEncCfg->waveCfg.cuLevelRCEnable = 1;//iValue;


    //if (WAVE_GetValue(fp, "EnHvsQp", &iValue) == 0)
    //    goto __end_parse;
    //else
        pEncCfg->waveCfg.hvsQPEnable = 1;//iValue;

    //if (WAVE_GetValue(fp, "HvsQpScaleDiv2", &iValue) == 0)
    //    goto __end_parse;
    //else
        if(EYENIX_AVC_TEST)
        pEncCfg->waveCfg.hvsQpScale = 2;//iValue;
        else
        pEncCfg->waveCfg.hvsQpScale = 0;//iValue;
    ...
    if (pEncCfg->VbvBufferSize == 0) {
        ...
        //    goto __end_parse;
        //else
            pEncCfg->VbvBufferSize = 3000;//iValue;
    ...
    //    goto __end_parse;
    //else
        pEncCfg->waveCfg.minQp = 8;//iValue;

    //if (WAVE_GetValue(fp, "MaxQp", &iValue) == 0)
    //    goto __end_parse;
    //else
        pEncCfg->waveCfg.maxQp = 51;//iValue;

    //if (WAVE_GetValue(fp, "MaxDeltaQp", &iValue) == 0)
    //    goto __end_parse;
    //else
        pEncCfg->waveCfg.maxDeltaQp = 30;//30;//iValue;

    //if (WAVE_GetValue(fp, "GOPSize", &iValue) == 0)
    //    goto __end_parse;
    //else
        pEncCfg->waveCfg.gopParam.customGopSize = 1;//iValue;

    //if (WAVE_GetValue(fp, "EnRoi", &iValue) == 0)
    //    goto __end_parse;
    //else
        pEncCfg->waveCfg.roiEnable = 0;//iValue;

    //if (WAVE_GetValue(fp, "GopPreset", &iValue) == 0)
    //    goto __end_parse;
    //else
        pEncCfg->waveCfg.gopPresetIdx = PRESET_IDX_CUSTOM_GOP;//PRESET_IDX_ALL_I;//;//3;//iValue;
    ...
    //    goto __end_parse;
    //else
        pEncCfg->waveCfg.chromaCbQpOffset = 0;//iValue;

    //if (WAVE_GetValue(fp, "CrQpOffset", &iValue) == 0)
    //        goto __end_parse;
    //else
        pEncCfg->waveCfg.chromaCrQpOffset = 0;//iValue;

    //if (WAVE_GetValue(fp, "RcInitialQp", &iValue) == 0)
    //    goto __end_parse;
    //else
        pEncCfg->waveCfg.initialRcQp = 30;//-1;////63;//iValue;

    //if (WAVE_GetValue(fp, "EnNoiseReductionY", &iValue) == 0)
    //    goto __end_parse;
    //else
        pEncCfg->waveCfg.nrYEnable = 0;//iValue;

    //if (WAVE_GetValue(fp, "EnNoiseReductionCb", &iValue) == 0)
    //    goto __end_parse;
    //else
        pEncCfg->waveCfg.nrCbEnable = 0;//iValue;

    //if (WAVE_GetValue(fp, "EnNoiseReductionCr", &iValue) == 0)
    //    goto __end_parse;
    //else
        pEncCfg->waveCfg.nrCrEnable = 0;//iValue;

    //if (WAVE_GetValue(fp, "EnNoiseEst", &iValue) == 0)
    //    goto __end_parse;
    //else
        pEncCfg->waveCfg.nrNoiseEstEnable = 0;//iValue;

    //if (WAVE_GetValue(fp, "NoiseSigmaY", &iValue) == 0)
    //    goto __end_parse;
    //else
        pEncCfg->waveCfg.nrNoiseSigmaY = 0;//iValue;

    //if (WAVE_GetValue(fp, "NoiseSigmaCb", &iValue) == 0)
    //    goto __end_parse;
    //else
        pEncCfg->waveCfg.nrNoiseSigmaCb = 0;//iValue;

    //if (WAVE_GetValue(fp, "NoiseSigmaCr", &iValue) == 0)
    //    goto __end_parse;
    //else
        pEncCfg->waveCfg.nrNoiseSigmaCr = 0;//iValue;

    //if (WAVE_GetValue(fp, "IntraNoiseWeightY", &iValue) == 0)
    //    goto __end_parse;
    //else
        if(EYENIX_AVC_TEST)
        pEncCfg->waveCfg.nrIntraWeightY = 0;//iValue;
        else
        pEncCfg->waveCfg.nrIntraWeightY = 7;//iValue;
    ...
    //else
        if(EYENIX_AVC_TEST)
        pEncCfg->waveCfg.nrIntraWeightCb= 0;//iValue;
        else
        pEncCfg->waveCfg.nrIntraWeightCb= 7;//iValue;

    //if (WAVE_GetValue(fp, "IntraNoiseWeightCr", &iValue) == 0)
    //    goto __end_parse;
    //else
        if(EYENIX_AVC_TEST)
        pEncCfg->waveCfg.nrIntraWeightCr = 0;//iValue;
        else
        pEncCfg->waveCfg.nrIntraWeightCr = 7;//iValue;

    //if (WAVE_GetValue(fp, "InterNoiseWeightY", &iValue) == 0)
    //    goto __end_parse;
    //else
        if(EYENIX_AVC_TEST)
        pEncCfg->waveCfg.nrInterWeightY = 0;//iValue;
        else
        pEncCfg->waveCfg.nrInterWeightY = 4;//iValue;

    //if (WAVE_GetValue(fp, "InterNoiseWeightCb", &iValue) == 0)
    //    goto __end_parse;
    //else
        if(EYENIX_AVC_TEST)
        pEncCfg->waveCfg.nrInterWeightCb = 0;//iValue;
        else
        pEncCfg->waveCfg.nrInterWeightCb = 4;//iValue;

    //if (WAVE_GetValue(fp, "InterNoiseWeightCr", &iValue) == 0)
    //    goto __end_parse;
    //else
        if(EYENIX_AVC_TEST)
        pEncCfg->waveCfg.nrInterWeightCr = 0;//iValue;
        else
        pEncCfg->waveCfg.nrInterWeightCr = 4;//iValue;

    //if (WAVE_GetValue(fp, "UseAsLongTermRefPeriod", &iValue) == 0)
    //    goto __end_parse;
    //else
        pEncCfg->waveCfg.useAsLongtermPeriod = 0;//iValue;

    //if (WAVE_GetValue(fp, "RefLongTermPeriod", &iValue) == 0)
    //    goto __end_parse;
    //else
        pEncCfg->waveCfg.refLongtermPeriod = 0;//iValue;
...
#ifdef EYENIX_TEST
        	pEncCfg->waveCfg.gopParam.picParam[i].picType      	= PIC_TYPE_P;
        	pEncCfg->waveCfg.gopParam.picParam[i].pocOffset   	= 1;
        	pEncCfg->waveCfg.gopParam.picParam[i].picQp        	= MIN(1, 1 + pEncCfg->waveCfg.intraQP);
        	pEncCfg->waveCfg.gopParam.picParam[i].numRefPicL0 	= 1;
        	pEncCfg->waveCfg.gopParam.picParam[i].refPocL0 		= 0;
        	pEncCfg->waveCfg.gopParam.picParam[i].temporalId 	= 0;
#else
    ...
    //else
    if(EYENIX_AVC_TEST)
        pEncCfg->waveCfg.rdoSkip = 1;//iValue;

    //if (WAVE_GetValue(fp, "LambdaScaling", &iValue) == 0)
    //    goto __end_parse;
    //else
    if(EYENIX_AVC_TEST)
        pEncCfg->waveCfg.lambdaScalingEnable = 1;//iValue;
    ...
    //else
    if(EYENIX_AVC_TEST)
        pEncCfg->waveCfg.transform8x8 = 1;//iValue;


    //if (WAVE_GetValue(fp, "SliceMode", &iValue) == 0)
    //    goto __end_parse;
    //else
        pEncCfg->waveCfg.avcSliceMode = 0;//iValue;

    //if (WAVE_GetValue(fp, "SliceArg", &iValue) == 0)
    //    goto __end_parse;
    //else
        pEncCfg->waveCfg.avcSliceArg = 0;//iValue;

    //if (WAVE_GetValue(fp, "IntraMbRefreshMode", &iValue) == 0)
    //    goto __end_parse;
    //else
        if(EYENIX_AVC_TEST)
        pEncCfg->waveCfg.intraMbRefreshMode = 0;//iValue;
    ...
    //else
        if(EYENIX_AVC_TEST)
        pEncCfg->waveCfg.intraMbRefreshArg = 1;//iValue;
    ...
    //else
        if(EYENIX_AVC_TEST)
        pEncCfg->waveCfg.mbLevelRc = 0;//iValue;
    ...
    //else
        if(EYENIX_AVC_TEST)
        pEncCfg->waveCfg.entropyCodingMode = 0;//1;//iValue;
    ...
    //    goto __end_parse;
    //else
        pEncCfg->waveCfg.monochromeEnable = 0;//iValue;

    //if (WAVE_GetValue(fp, "StrongIntraSmoothing", &iValue) == 0)
    //    goto __end_parse;
    //else
        if(EYENIX_AVC_TEST)
        pEncCfg->waveCfg.strongIntraSmoothEnable = 0;//iValue;
        else
        pEncCfg->waveCfg.strongIntraSmoothEnable = 1;//iValue;


    //if (WAVE_GetValue(fp, "RoiAvgQP", &iValue) == 0)
    //    goto __end_parse;
    //else
        pEncCfg->waveCfg.roiAvgQp = 0;//iValue;

    //if (WAVE_GetValue(fp, "WeightedPred", &iValue) == 0)
    //    goto __end_parse;
    //else
        pEncCfg->waveCfg.weightPredEnable = 0;//iValue & 1;

    //if (WAVE_GetValue(fp, "EnBgDetect", &iValue) == 0)
    //    goto __end_parse;
    //else
        pEncCfg->waveCfg.bgDetectEnable = 0;//iValue;

    //if (WAVE_GetValue(fp, "BgThDiff", &iValue) == 0)
    //    goto __end_parse;
    //else
        pEncCfg->waveCfg.bgThrDiff = 8;//iValue;
