...
Int32 GetEncOpenParamDefault(EncOpenParam *pEncOP, TestEncConfig *pEncConfig)
{
    ...
    pEncOP->frameRateInfo           = 3;//12;//30;//10;//12;//15;//30;
    ...
    pEncOP->gopSize                 =	1; //30;      // only first picture is I
    ...
    if( bitFormat == STD_MPEG4 ) {
...
#ifdef EYENIX_TEST
        /* for CMD_ENC_SEQ_GOP_PARAM */
        param->gopPresetIdx     = PRESET_IDX_CUSTOM_GOP;//PRESET_IDX_ALL_I;//PRESET_IDX_IBBBP;

#else
		/* for CMD_ENC_SEQ_GOP_PARAM */
		param->gopPresetIdx     = PRESET_IDX_ALL_I;//PRESET_IDX_IBBBP;
#endif
        /* for CMD_ENC_SEQ_INTRA_PARAM */
        param->decodingRefreshType = 1;
        param->intraPeriod         = 3;//6;//12;//15;//30;//5;//30;//15;//29;
        param->intraQP             = 0;
        ...
        for (i= 0; i<param->gopParam.customGopSize; i++) {
#ifdef EYENIX_TEST
        	param->gopParam.picParam[i].picType      	= PIC_TYPE_P;
        	param->gopParam.picParam[i].pocOffset   	= 1;
        	param->gopParam.picParam[i].picQp        	= MIN(1, 1 + param->intraQP);
        	param->gopParam.picParam[i].numRefPicL0 	= 1;
        ...
        if (bitFormat == STD_AVC && pEncOP->picHeight == 1080)
        {
            // In case of AVC encoder, when we want to use unaligned display width(For example, 1080),
            // frameCroppingFlag parameters should be adjusted to displayable rectangle
            if (pEncConfig->rotAngle != 90 && pEncConfig->rotAngle != 270) // except rotation
